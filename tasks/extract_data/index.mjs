...
functions.http('extract_opa_assessments', async (req, res) => {
  ...
  const bucket = storage.bucket(BUCKET_NAME);
  const file = bucket.file('opa_assessments/opa_assessments.csv');
  ...
  console.log(`Successfully extracted opa_assessments to gs://${BUCKET_NAME}/${file.name}`);
  res.send(`Successfully extracted opa_assessments to gs://${BUCKET_NAME}/${file.name}`);
});
...
functions.http('prepare_opa_assessments', async (req, res) => {
  ...
  const rawFile = storage.bucket(RAW_BUCKET_NAME).file('opa_assessments/opa_assessments.csv');
  const tableFile = storage.bucket(TABLE_BUCKET_NAME).file('opa_assessments/data.jsonl');
